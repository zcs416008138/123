...
#计算入炉钢坯数量和温度插入82数据库fullbarroll.gangpiNumber。同时区分是哪个班组的，和他是夜中白班中的哪一个
def inFurnaceBillet():
    global POOL
    try:
        # 从数据库里面读取出来原来的最近一笔的钢坯号。
        sql0 = "SELECT * FROM fullbarroll.gangpiNumber order by timeID desc limit 1;"
        ...
        # 从数据库读取处原始数据
        sql ="SELECT id,timeID,  furOutsid_furOutsid_Efwd_VALUE as ruluRtFwd," \
              " furnacePLC_metaltempinfurnace_metaltemptinfunc_VALUE as tempinfunc," \
              "metltemp_tempoutfunc_metaltempoutfunc_VALUE as outTemp," \
              "furOutsid_furOutsid_chugangQianJin_VALUE as chugangfwd, " \
              "furOutsid_furOutsid_chugangHouTui_VALUE    as chugangrev, " \
              "furOutsid_furOutsid_tuigangjiBkward_VALUE as Treve1," \
              "furOutsid_furOutsid_tuigangjiFWD_VALUE as Tq1fwd," \
              "mill_mill_mill03_rmstop_VALUE as cmillstart ," \
              "mill_mill_mill02_chuluHveStel_VALUE as chuluHveStel, " \
              "mill_mill_mill02_std1mis_VALUE as std1hstel, " \
               "mill_mill_mill02_chuluFWD_VALUE  " \
              "FROM fullbarroll.rulugenzong where timeID > " + "\'" + str(recentTime) + "\'" + "order by timeID asc limit 80000;"
        print("sql111111=",sql)
        # 调取数据，将数据从数据库里面读取出来。
        ...
        chulu1run = list(result1[0:, 12])  # 出炉辊道运行信号
        # print(len(timeID))
        # print(timeID[0])
        ...
        for x in range(2, len(T_reverse) - 5):
        ...
        try:
            if readGangpiSucc == 1:  # 如果读取到钢坯温度数据了，要往数据库里面插入新数据那么就将这个数据更新
                for x in range(len(tempIn)):  # 开始循环写数据
                    ...
                    if recentId == 0:
                        sqldel = "UPDATE  fullbarroll.gangpiNumber SET timeID='" + str(timeID1[x]) + "',tempInFunc = " + str(
        ...
        try:
            ...
            if readGangpiSucc == 0:  # 如果从原始数据库里面的数据没有要插入的，那么先查看gangpiNumber 第一个数据是否为0.
                ...
                if arr1[0][3] != 0 and timeCha >= 49000:  # 如果从数据库里面读取出来的第一个数据不是0， 并且当前的时间差和数据库里面的时间差大于49000了，那么把第一个数据插成0
                    ...
                    insertSql = "INSERT INTO fullbarroll.gangpiNumber (timeID, tempInFunc, idNumber)" \
                                "values( %s,%s,%s)"
                    ...
                    sqldel = "UPDATE  fullbarroll.gangpiNumber SET timeID='" + str(timeID[-1]) + "',tempInFunc = " + str(
...
def tempOutFurnace1():
    ...
    try:
        ...
        # 从数据库里面读取出来原来的最近一笔的钢坯号。
        sql0 = "SELECT * FROM fullbarroll.tempOutFunc order by timeID DESC limit 1;"
        arr0 = readMysql(POOL, sql0)
        actId = arr0[0][0]  # 当前ID号
        ...
        sql = "SELECT id,timeID,  furOutsid_furOutsid_Efwd_VALUE as ruluRtFwd, " \
              ...
              "metltemp_tempoutfunc_metaltempoutfunc_VALUE as outTemp, " \
              "mill_mill_mill02_std1mis_VALUE as std1hstel," \
              "mill_mill_mill02_std2mis_VALUE as std2hstel " \
              "FROM fullbarroll.rulugenzong where timeID > " + "\'" + str(
        ...
        if len(outTemp) == 0 and tempread == 0:
            sqlupdate = "UPDATE  fullbarroll.tempOutFunc SET timeID='" + str(timeID[-1]) + "',tempInFunc = " + str(
                0) + ",idNumber = " + str(0) + " where id = " + str(actId)
            updatesql(POOL, sqlupdate)
        # 如果是读取出来的数据第一个是已经是温度数据是0了，那么就更新他的timeID时间
        elif len(outTemp) == 0 and tempread != 0:
            sqlOutTemp = "INSERT INTO fullbarroll.tempOutFunc(timeID, tempOutFunc, idNumber)values( %s,%s,%s)"
            data30 = [timeID[-1], 0, 0]
        ...
        # 如果钢坯实际的数据不为0,说明读取到钢坯数据了。
            ...
            for x in range(0, int(len(outTemp) / 2) - 1):
                ...
                if tempread == 0 and outTempGangpiStatu == 0:
                    sqlupdate = "UPDATE  fullbarroll.tempOutFunc SET timeID='" + str(timeID3) + "',tempInFunc = " + str(
                ...
                # 其他情况直接写入数据
                    ...
                    # print("else=")
                    sqlOutTemp = "INSERT INTO fullbarroll.tempOutFunc(timeID, tempOutFunc, idNumber,maxTempOut, minTempOut)values( %s,%s, %s,%s,%s)"
...
def std1HeadTailCurrent():
    ...
    try:
        # 从数据库里面读取出来原来的最近一笔的钢坯号。
        sql0 = "SELECT * FROM fullbarroll.standstel where millID = 1 order by timeID desc limit 1;"
        arr0 = readMysql(POOL, sql0)
        actId = arr0[0][0]  # 当前ID号
        recentTime = arr0[0][1]  # 获取最新一笔数据库的时间值
        ...
        sql = "SELECT id,timeID,  furOutsid_furOutsid_Efwd_VALUE as ruluRtFwd," \
              ...
              "mill_mill_mill02_std1mis_VALUE as std1hstel, " \
              "mill_mill_mill02_chuluFWD_VALUE , " \
              "mill_mill_mill03_std1cur_VALUE , " \
              "mill_mill_mill01_rollintTime_VALUE , " \
              "mill_mill_mill02_guige_VALUE  " \
              "FROM fullbarroll.rulugenzong where timeID > " + "\'" + str(recentTime) + "\'" + "order by timeID asc limit 80000;"
        ...
        if len(std1h0_1) == 0 and single == 0:  # 如果从原始数据库里面读取出来的数是0，并且从standstel表里面读取出来的single信号是0.那么更新
            sqlupdate = "UPDATE  fullbarroll.standstel SET timeID='" + str(timeID[-1]) + " where id = " + str(actId)
            updatesql(POOL, sqlupdate)
        # 如果列表std1h0_1 是[]，而读取出来的single!=0.那么就插入一条single为0 的数据
        if len(std1h0_1) == 0 and single != 0 and len(timeID) > 78000:
            sqlstd1 = "INSERT INTO fullbarroll.standstel(timeID, single)values( %s,%s)"
            data30 = [timeID[-1], 0]
            print("data3=", data30)
        ...
        if len(std1h0_1) != 0:
            for x in range(len(std1h0_1)):
                ...
                if single == 0 and std1single == 0:
                    sqlupdate = "UPDATE fullbarroll.standstel SET timeID='" + str(timeIDstd1) + "',instd1time = '" + str(
                ...
                # 如果不是0，那么直接将数据插入即可
                    ...
                    #print("else = ")
                    sqlstdinsert = "INSERT INTO fullbarroll.standstel(timeID, instd1time, outstd1time, avgcurrent, millID, single,rolltime,guige )values( %s,%s, %s,%s, %s,%s,%s,%s)"
...
# 此段程序用于将tempOutFurnace1计算出来的出炉时间给插入到数据库里面，让其跟进钢相对应（表fullbarroll.tempOutFunc里面的数据插入fullbarroll.gangpiNumber）
def insertTempOurFunc_InsertGangPiPosition():
    global POOL
    try:
        # 从数据库gangpiNumber里面读取出来原来的最近一笔的钢坯号。
        sql0 = "SELECT * FROM fullbarroll.gangpiNumber order by timeID desc limit 20000;"
        arr0 = readMysql(POOL, sql0)
        actId = arr0[:, 0]  # 当前ID号
        ...
        # 从数据库里面查询处理的出炉温度数据
        Tsql0 = "SELECT * FROM fullbarroll.tempOutFunc order by timeID DESC limit 80000;"
        ...
        for x1 in range(len(recentTime) - 157):
            if actId[x1 + 10] <= actId[0] - yureNum:
                ...
                break
            sqlyure = "update fullbarroll.gangpiNumber set timeInyure = '" + str(recentTime[x1]) + "'  where id = " + str(
                actId[x1 + 10])
            print("sqlyure111111111111=", sqlyure)
            updatesql(POOL, sqlyure)

            # 将钢坯进加热段时间找出来
            sqljiare = "update fullbarroll.gangpiNumber set timeInJiare = '" + str(recentTime[x1]) + "'  where id = " + str(
                actId[x1 + 58])
            updatesql(POOL, sqljiare)
            print("sqljiare222222222222=", sqljiare)

            # 将钢坯进均热段时间找出来
            sqljunre = "update fullbarroll.gangpiNumber set timeInJunre = '" + str(recentTime[x1]) + "'  where id = " + str(
                actId[x1 + 105])
            updatesql(POOL, sqljunre)
            print("sqljunre=", sqljunre)

            # 将钢坯出炉的时间找出来
            sqloutfunc = "update fullbarroll.gangpiNumber set timeOutFuc = '" + str(
...
def insertStd1ToGangPiNumber():
    ...
    global POOL
    try:
        # 首先查看1H钢坯的数据，fullbarroll.gangpiNumber里面数据更新到哪个位置了。
        sql1H = "SELECT * FROM fullbarroll.gangpiNumber  where instd1 is not Null order by timeID DESC"
        ...
        sql0 = "SELECT * FROM fullbarroll.gangpiNumber  where timeID >= '" + str(
        ...
        # sql0 = "SELECT * FROM fullbarroll.standstel where millID = 1 order by timeID desc limit 1;"
        sql1 = "SELECT * FROM fullbarroll.standstel where timeID >='" + str(timeinstd1) + "' order by timeID ASC limit 80000"
        print("sql1=", sql1)
                ...
                for y in range(len(Sinstd1time0) - 1):
                    if timeOutFucNum[x] > Sinstd1time0[y] and timeOutFucNum[x] < Sinstd1time0[y + 1]:
                        ...
                        if Sinstd1time0[y + 1] != comptime:
                            ...
                            sql1 = "update  fullbarroll.gangpiNumber  set " + "instd1 = '" + str(
...
def caculateGangPiAvg():
    ...
    try:
        # 查询数据，看哪些数据没有插入新的数据
        sqlnull = "SELECT * FROM fullbarroll.gangpiNumber WHERE yureAvgTemp is not null order by timeID desc limit 10"
        arrynull = readMysql(POOL, sqlnull)
        if arrynull.size == 0:
            sqlnull0 = "SELECT * FROM fullbarroll.gangpiNumber  order by timeID asc limit 10"
            arrynull0 = readMysql(POOL, sqlnull0)
            timenull = arrynull0[0][1]
        else:
            timenull = arrynull[0][1]  # 需要的时间
        sqltimeout = "SELECT * FROM fullbarroll.gangpiNumber where timeOutFuc is not null order by timeID desc limit 10"
        arryout = readMysql(POOL, sqltimeout)
        timeout = arryout[0][7]  # 需要的时间
        # 查看gangpiNumber 数据库
        sql0 = "SELECT * FROM fullbarroll.gangpiNumber where timeID between '"+ str(timenull)+"' and '"+ str(timeout)+"' order by timeID asc limit 300000"
        #sql0 = "SELECT * FROM fullbarroll.gangpiNumber where timeID between '2022-05-30 16:53:50' and '2022-06-01 14:59:12' order by timeID DESC limit 300000"
        print("sql0=", sql0)
        arr0 = readMysql(POOL, sql0)
        actId = arr0[:, 0]  # 当前ID号
        recentTime = arr0[:, 1]  # 获取最新一笔数据库的时间值
        ...
        for data in arr0:
            ...
            if data[7] != None:
                ...
                yure1 = miaojiqujianqlz(dataTranfer)
                # print("yure1=",yure1)
                ...
                # print("dataTranfer=",dataTranfer)
                jiare2 = miaojiqujianqlz(dataTranfer)
                ...
                # print("dataTranfer=",dataTranfer)
                junre3 = miaojiqujianqlz(dataTranfer)
                # print("junre3=",junre3)
                ...
                # sqlinsert = ""UPDATE  fullbarroll.gangpiNumber SET timeID='" + str(huaguiContainTime) + "',instd1time = '" + str(instd1time) + "',outstd1time = '" + str(outstd1time) +"',avgcurrent = " + str(avgcur) +",millID = " + str(1) + ",single = " + str(1) +",rolltime = " + str(rolltimelist[x]) +",guige = " + str(guigelist[x])+  " where id = " + str(actId) "
                sqlinsert = "UPDATE  fullbarroll.gangpiNumber SET huaguiContainTime =" + str(
...
def insertOutTemp():
    ...
    timeInstd1Compare = 0
    try:
        sqlnull = "SELECT * FROM fullbarroll.gangpiNumber WHERE tempOutFunc is not null order by timeID desc limit 10"
        arrynull = readMysql(POOL, sqlnull)
        if arrynull.size == 0:
            sqlnull0 = "SELECT * FROM fullbarroll.gangpiNumber  order by timeID asc limit 10"
            arrynull0 = readMysql(POOL, sqlnull0)
        ...
        Tsql0 = "SELECT * FROM fullbarroll.tempOutFunc  where timeID >= '" + str(
            timenull) + "'  order by timeID ASC limit 80000;"
                ...
                if y < len(timeInStd1) - 1:
                    ...
                    if timeOutTemp[x] > timeInStd1[y] and timeOutTemp[x] < timeOutStd1[y + 1]:
                        ...
                        if timeInStd1[y] != timeInstd1Compare:
                            ...
                            sql1 = "update  fullbarroll.gangpiNumber  set " + " tempOutFunc = " + str(
                    ...
                    elif timeOutTemp[x] > timeInStd1[y] and timeOutStd1[y + 1] == 0:
                        # 避免重复插入数据
                        if timeInStd1[y] != timeInstd1Compare:
                            print("timeOutTemp=", timeOutTemp[x])
                            print("timeInStd1[y]=", timeInStd1[y])
