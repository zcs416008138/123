...
leng1 = 80000
sqlread ="SELECT * FROM shaft8b.shaftzone where zoneID= 1 order by timeID DESC limit "+str(leng1) # 南为1
...
sqlread ="SELECT * FROM shaft8b.shaftmain order by timeID DESC limit "+str(leng1)
dataRead2 = readMysqlToArry(sqlread,leng1)

isAuto = dataRead2[:,11]

sqlread ="SELECT * FROM shaft8b.shaftzone where zoneID= 2 order by timeID DESC limit "+str(leng1) # 北为2
dataRead1 = readMysqlToArry(sqlread,leng1)
