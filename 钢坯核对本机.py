...
def gangpi (EID,kuming,kuming2):
    while 1:
        # 查看要插入的数据库中最后一笔数据的时间
        sql0 = "SELECT * FROM " + kuming2 + " order by End desc limit 1"
        # print("sql0=",sql0)
        arr1 = readMysql(POOL2, sql0)           # 对本地数据库进行链接
        # print("arr1=",arr1)
        if len(arr1) == 0:
        ...
        # print("time3_quzheng : ",time3_quzheng)
        # 处理成用来确定 EID 的时间范围
...
def barroll():
    gangpi(['bfa0bff2'], 'barroll.gangpiNumber', 'barroll.billet_checking')  # eid,调取数据的库名，要插入的库名

def newbar():
    gangpi(['657df568'], 'newbar.gangpiNumber', 'newbar.billet_checking')  # eid,调取数据的库名，要插入的库名

def fullbarroll():
    gangpi(['9fe9215c'], 'fullbarroll.gangpiNumber', 'fullbarroll.billet_checking')  # eid,调取数据的库名，要插入的库名
